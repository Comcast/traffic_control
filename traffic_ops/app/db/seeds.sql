...
SET @param_id := (select id from parameter where name = 'DNSRoutingName' and value = 'tr');
INSERT IGNORE INTO profile_parameter (profile, parameter )
  SELECT id, @param_id
  FROM profile
  where name like 'CCR%' ;

SET @param_id := (select id from parameter where name = 'HTTPRoutingName' and value = 'edge');
INSERT IGNORE INTO profile_parameter (profile, parameter )
  SELECT id, @param_id
  FROM profile
  where name like 'CCR%' ;