		...
		try {
			...
			if (cacheRegister != null) {
				for (Steering steering : steeringRegistry.getAll()) {
					final DeliveryService deliveryService = cacheRegister.getDeliveryService(steering.getId());

					if (deliveryService == null) {
						LOGGER.warn("Steering data from " + dataBaseURL + " contains delivery service id reference '" + steering.getId() + "' that's not in cr-config");
						continue;
					}

					for (SteeredDeliveryService steeredDeliveryService : steering.getSteeredDeliveryServices()) {
						deliveryService.addSteeredDeliveryService(steeredDeliveryService);
					}
				}
			}
			return true;
