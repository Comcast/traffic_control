...
public class SteeringTest {
	...
	Map<String, String> steeredDeliveryServicesDomains = new HashMap<String, String>();
	Map<String, Integer> steeredDeliveryServicesWeights = new HashMap<String, Integer>();
	...
	public void before() throws Exception {
		...
		steeringDeliveryServiceId = steeringNode.get("id").asText();
		Iterator<JsonNode> steeredDeliveryServices = steeringNode.get("steeredDeliveryServices").iterator();
		while (steeredDeliveryServices.hasNext()) {
			JsonNode steeredDeliveryService = steeredDeliveryServices.next();
			String steeredId = steeredDeliveryService.get("id").asText();
			Integer steeredWeight = steeredDeliveryService.get("weight").asInt();
			steeredDeliveryServicesWeights.put(steeredId, steeredWeight);
			steeredDeliveryServicesDomains.put(steeredId, "");
		}
		...
		while (deliveryServices.hasNext()) {
			...
			if (steeredDeliveryServicesDomains.containsKey(dsId)) {
				steeredDeliveryServicesDomains.put(dsId, jsonNode.get("deliveryServices").get(dsId).get("domains").get(0).asText());
			}
		}

		assertThat(steeringDeliveryServiceId, not(nullValue()));
		assertThat(steeredDeliveryServicesDomains.isEmpty(), equalTo(false));

		for (String deliveryServiceId : steeredDeliveryServicesDomains.keySet()) {
			...
			while (cacheIds.hasNext()) {
				...
				if (cacheNode.get("deliveryServices").has(deliveryServiceId)) {
					...
					validLocations.add("http://" + cacheId + "." + steeredDeliveryServicesDomains.get(deliveryServiceId) + portText + "/stuff?fakeClientIpAddress=12.34.56.78");
				}
			}
		}
	...
	@Test
	public void itUsesBypassFiltersForSteering() throws Exception {
	...
	public void itUsesWeightedDistributionForRequestPath() throws Exception {
		int count = 0;
		for (int weight : steeredDeliveryServicesWeights.values()) {
			count += weight;
		}
		...
		for (String steeredId : steeredDeliveryServicesWeights.keySet()) {
		...
		for (String id : steeredDeliveryServicesWeights.keySet()) {
			expectedHitRates.put(id, steeredDeliveryServicesWeights.get(id) / totalWeight);
		}
